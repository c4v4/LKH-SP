...
GainType Penalty() {
    ...
    if (Swaps && cava_PetalsData) {
        ...
        while (NC_idx) {
            ...
            if (N->Id > DimensionSaved) N -= DimensionSaved;

            CostSum = N->prevCostSum;
            DemandSum = N->prevDemandSum;
            petalP = N->prevPenalty;
            do {
                if ((DemandSum += N->Demand) > Capacity) petalP += DemandSum - Capacity;
                if (CostSum < N->Earliest) CostSum = N->Earliest;
                if (CostSum > N->Latest) petalP += CostSum - N->Latest;
                if ((P + petalP) > oldPenaltySum || ((P + petalP) == oldPenaltySum && CurrentGain <= 0)) return CurrentPenalty + (CurrentGain > 0);

                CostSum += N->ServiceTime;

                NextN = (Forward ? SUC(N) : PRED(N));

                int cost = (Forward ? SUC_COST(N) : PRED_COST(N)) - N->Pi - NextN->Pi;
                CostSum += cost / Precision;

                N = (Forward ? SUC(NextN) : PRED(NextN));
            } while (N->DepotId == 0);

            if (CostSum > Depot->Latest) petalP += CostSum - Depot->Latest;
        ...
        if (!CurrentPenalty || P < oldPenaltySum || (P == oldPenaltySum && CurrentGain > 0)) {
        ...
        if (!cava_NodeCache) cava_NodeCache = (Node **)calloc(Salesmen + 1, sizeof(Node *));

        if (!cava_PetalsData) cava_PetalsData = (RouteData *)calloc(Salesmen + 1, sizeof(RouteData));
        P = Penalty_Old();
        if (P < CurrentPenalty || (P == CurrentPenalty && CurrentGain > 0)) update_Penalty();
...
int setup_Penalty() {
    ...
    for (SwapRecord *s = SwapStack + Swaps - 1; s >= SwapStack; --s) {
        ...
        if ((N = setup_node_CVRPTW(s->t1)) != NULL) cava_NodeCache[touched_routes++] = N;

        if ((N = setup_node_CVRPTW(s->t4)) != NULL) cava_NodeCache[touched_routes++] = N;
...
void route_min_node_update(Node *t) {
    if (!t->PetalId->minNode || t->PetalId->minNode->PetalRank > t->PetalRank) t->PetalId->minNode = t;
}
...
GainType Penalty_Old() {
    ...
    do {
        ...
        if (CostSum > Depot->Latest && ((P + (petalP += CostSum - Depot->Latest)) > CurrentPenalty || ((P + petalP) == CurrentPenalty && CurrentGain <= 0))) {
            StartRoute = CurrentRoute;
            return CurrentPenalty + (CurrentGain > 0);
        }
        P += petalP;
