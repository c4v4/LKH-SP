...
void MTSP_WriteSolution(char *FileName, GainType Penalty, GainType Cost) {
    ...
    if (TraceLevel >= 1) printff("Writing MTSP_SOLUTION_FILE: \"%s\" ... ", FullFileName);
    ...
    do {
        ...
        do {
            ...
            Size++;
            if (NextN->Id > DimensionSaved) NextN = Forward ? NextN->Suc : NextN->Pred;
            N = NextN;
        } while (N->DepotId == 0);
        if (N->DepotId <= ExternalSalesmen) fprintf(SolutionFile, "(#%d)  Cost: " GainFormat "\n", Size, Sum / Precision);
        else
