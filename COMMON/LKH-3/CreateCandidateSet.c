...
void CreateCandidateSet() {
    ...
    } else  */
    {
        if (CandidateSetType != DELAUNAY && CandidateSetType != POPMUSIC && MaxCandidates > 0) {
            ...
            if (CandidateSetType == DELAUNAY) CreateDelaunayCandidateSet();
            else
    ...
    if (TraceLevel >= 1) {
        ...
        if (Optimum != MINUS_INFINITY && Optimum != 0 &&
            (ProblemType != CCVRP && ProblemType != TRP && ProblemType != MLP && MTSPObjective != MINMAX && MTSPObjective != MINMAX_SIZE))
            printff(", Gap = %0.2f%%", 100.0 * (Optimum - LowerBound) / Optimum);
        if (!PiFile) printff(", Ascent time = %0.2f sec.", fabs(GetTime() - EntryTime));
        printff("\n");
    }
    ...
    if (Optimum != MINUS_INFINITY && Optimum != 0 &&
        ...
        MaxAlpha = A;
    if (CandidateSetType == DELAUNAY || CandidateSetType == POPMUSIC || MaxCandidates == 0) OrderCandidateSet(MaxCandidates, MaxAlpha, CandidateSetSymmetric);
    else
    ...
    ResetCandidateSet();
    if (MaxTrials > 0 || (InitialTourAlgorithm != SIERPINSKI && InitialTourAlgorithm != MOORE)) {
        Na = FirstNode;
        do {
            if (!Na->CandidateSet || !Na->CandidateSet[0].To) {
                if (MaxCandidates == 0) eprintf("MAX_CANDIDATES = 0: Node %d has no candidates", Na->Id);
                else
                    eprintf("Node %d has no candidates", Na->Id);
            }
        } while ((Na = Na->Suc) != FirstNode);
        if (!CandidatesRead && SubproblemSize == 0) WriteCandidates();
    }
    if (_C == C_EXPLICIT) {
        Na = FirstNode;
        do
